 ...
 *
 */
public abstract class Shapes implements HandleingEvents{
    ...
    public void applyTransform() {
        for (int i = 0; i < pointsOfShape.size(); i++) {
            ...
            pointsOfShape.set(i, new Point2D(p.getX() + transform.getX(), p.getY() + transform.getY()));
        }
    ...
    public Point2D getTopLeftPoint() {
        ...
        double x = pointsOfShape.get(0).add(transform).getX();
        double y = pointsOfShape.get(0).add(transform).getY();

        for (int i = 1; i < pointsOfShape.size(); i++) {
            Point2D p = pointsOfShape.get(i).add(transform);
            if (p.getX() < x)
                x = p.getX();
            if (p.getY() < y)
                y = p.getY();
